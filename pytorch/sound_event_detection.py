...
def add_text_to_video(framewise_output, labels, video_path, out_video_path):
    ...
    out = cv2.VideoWriter(tmp_video_path, cv2.VideoWriter_fourcc(*'MJPG'),
    ...
    rect_coord = (int(0.5*frame_width), int(0.4*frame_height))
    ...
    while(True):
        ...
        cv2.rectangle(frame, (0, 0), rect_coord, (255, 255, 255), -1)
        ...
        for k in range(0, topk):
            ...
            bottomLeftCornerOfText = (5, int(0.05*frame_width + k * 20))
            fontScale              = 0.5
            fontColor              = (0,0,255)
            lineType               = 2
